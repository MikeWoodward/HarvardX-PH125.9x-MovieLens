...
* edx - 90% of the data. This data was used for training and regularization. All of the results in the exploratory data analysis, model building and training, and regularizations section use this data *only*. 

* validation - 10% of the data. This is a hold out data set and was *only* used for the final model evaluation. The only section in this report that uses this data is the Results section.

For model development and testing, I partioned the edx group into two sub-groups (again using randomization and a seed of 1):
...
There are a large number of users, movies, and ratings. This suggests the modeling computational burden will be high. The high average number of ratings per user and ratings per movie suggests I should look closely at the underlying distributions, there may be some structure relevant to modeling.

The most obvious distribution is the number of reviews by rating, shown in Figure 1 (below).

![Ratings histogram](data/edx_ratings_histogram.png)


The quantization and the upper and lower bounds are obvious, but it's also obvious that the distribution is *not* uniform, which suggests there are underlying factors that affect ratings. Note  the most common rating (mode) is 4 and that integer ratings are more common than half scores. I'll return to this topic in the discussion section.
...
![Ratings histogram](data/edx_movies_histogram.png)

Figure 3 plots the user count vs mean rating. Again, there's a very strong user effect on rating and as a result, I need to include this feature in my model. Note that it's an approximately normal distribution and its shape is different from the movie effect, suggesting different dynamics are at work.

![Ratings histogram](data/edx_users_histogram.png)
...
However, they can be combined in several ways. In the edx data, there are `r edx_distinct_genre_combos` unique values for the genres field. As a first approximation, it seems reasonable that combined genres (e.g 'Action | Adventure') may behave differently from the individual genres (e.g. 'Action', 'Adventure'). I'm going to confine my analysis to combined genres, specifically, the contents of the genres column.
...
![Ratings histogram - 50,000 ratings and more](data/edx_genres_ratings_micro.png)


This is just the top viewed movies, but what about movies with less than 50,000 reviews? There are `r edx_distinct_genre_combos` genres, which means I can't display the grenre names on the axis. With the full data set, there's a strong genres effect, but some genres have a wide confidence interval, which is something I'll come back to in the discussion.

![Ratings histogram - full genres set](data/edx_genres_ratings_macro.png)

The movie release year is another possible feature. I extracted release year from the title and plotted the mean rating by year (with 95% confidencen interval) below (Figure 6). Ratings are dependent on release year, which is very clear after about 1970. Prior to 1970, the pattern is less obvious and I'll come back to this point in the discussion. For now, I'll add movie release year as another model feature.
...
The chart shows that, as expected, oldert movies have better reviews. I will include this feature in my model.
...
* $f(ry_{u,i})$ - where $f$ is a smooth function of $ry_{u,i}$. This represents the mean rating for the release year feature. Again for brevity, I will label this term $b_{ry}$
...
The code is a straightforward implementation of this algorithm, for example, here's my code to calculate the genres effect.
...
scores <- rbind(scores, c("Genres", rmse_genres))
```

I'll use RMSE as my metric of success. RMSE is defined by the function $RMSE = \sqrt{\frac{1}{N}\sum_{u,i}{(\hat y_{u,i} - y_{u,i})^2}}$, where $N$ is the number of recommendations, $\hat y_{u,i}$ is the predicted rating and $y_{u,i}$ is the actual rating. This is the course mandated measure.
...
* Mean, movie effects, user effects, genre effects, and release review effects.

* Mean, movie effects, user effects, genre effects, release review, and release year effects.

At each stage, I calculated the RMSE using the test subset of the edx data set. Here are the results.
...
As expected, adding more features decreases RMSE, taking me below the course target of `r rmse_target` However, I may have overfitted the model. I need to regularize the model prior to the final analysis.
...
The final stage is using the entire edx data to train the model and the validation (hold out) data to calculate the final RMSE. 
...
As I pointed out earlier, ratings are quantized and the number of half star ratings is less than the number of whole star ratings. It may be worth investigating whether different factors drive full and half star ratings, for example, some users may rate on full stars alone.logit!!!! k-means

When modeling genres, I considered *all* genres for a movie. But there may be specific genre effects, for example "Comedy" movies might be more highly reviewed than 'Western' movies. There may well also be an interaction between genre and year, for example, there are fewer 'Western' movies produced now than there were in the 1940's.

The release year chart (Figure 6) showed little variation before about 1970. A !!!!
